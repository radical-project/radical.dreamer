...
# again with other arguments to build sdist and wheel packages.  We must *not*
...
def get_version(_mod_root):
    ...
    try:
        ...
        # i.e., if 'git rev-parse --show-prefix' returns an empty string --
        # otherwise we get confused if the ve lives beneath another repository,
        # and the pip version used uses an installed tmp dir in the ve space
        ...
        out, _, ret = sh_callout(
            ...
            '2>/dev/null ; echo $tag@$branch' % root)
...
# ------------------------------------------------------------------------------
# get version info -- this will create VERSION and src_root/VERSION
...
if sys.hexversion < 0x03060000:
    raise RuntimeError('%s requires Python 3.6 or newer' % name)
...
base = 'share/%s' % name
data_files = [
    ('%s/examples/' % base, glob.glob('examples/*'))
]
...
setup_args = {
    ...
    'version'           : version,
    'description'       : 'Dynamic Runtime and Execution Adaptive Middleware '
                          'EmulatoR',
    'author'            : __author__,
    'author_email'      : __email__,
    'maintainer'        : __author__,
    'maintainer_email'  : __email__,
    'url'               : 'https://github.com/radical-project/%s' % name,
    'license'           : __license__,
